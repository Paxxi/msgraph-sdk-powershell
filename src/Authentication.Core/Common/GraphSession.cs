...
namespace Microsoft.Graph.PowerShell.Authentication
{
    ...
    public class GraphSession : IGraphSession
    {
        ...
        /// </summary>
        public string SelectedProfile { get; set; } = Core.Constants.DefaultProfile;
            ...
            get
            {
                try
                {
                    ...
                    try
                    {
                        if (null == _instance)
                        {
                            throw new InvalidOperationException(Core.ErrorConstants.Codes.SessionNotInitialized);
                        }
                        return _instance;
                    }
                    finally
                ...
                {
                    throw new InvalidOperationException(Core.ErrorConstants.Codes.SessionLockReadRecursion, lockException);
                }
                catch (ObjectDisposedException disposedException)
                {
                    throw new InvalidOperationException(Core.ErrorConstants.Codes.SessionLockReadDisposed, disposedException);
                }
            }
        }
        ...
        public static void Initialize(Func<GraphSession> instanceCreator, bool overwrite)
        {
            ...
            {
                throw new InvalidOperationException(Core.ErrorConstants.Codes.SessionLockWriteRecursion, lockException);
            }
            catch (ObjectDisposedException disposedException)
            {
                throw new InvalidOperationException(Core.ErrorConstants.Codes.SessionLockWriteDisposed, disposedException);
            }
        }
        ...
        public IPSGraphOutputWriter OutputWriter
        {
            get
            {
                ...
                {
                    throw new InvalidOperationException(Core.ErrorConstants.Codes.OutputNotInitialized);
                }
