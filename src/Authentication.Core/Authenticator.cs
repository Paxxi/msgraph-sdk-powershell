        ...
        public static async Task<IAuthContext> AuthenticateAsync(IAuthContext authContext, bool forceRefresh, CancellationToken cancellationToken, Action fallBackWarning = null)
        {
...
#else
                httpRequestMessage.Options.Set(new HttpRequestOptionsKey<GraphRequestContext>(typeof(GraphRequestContext).ToString()), graphRequestContext);
#endif
            ...
            catch (AuthenticationException authEx)
            {
                ...
                if (authEx.InnerException is TaskCanceledException && cancellationToken.IsCancellationRequested)
                {
                    ...
                    throw new Exception(string.Format(CultureInfo.CurrentCulture, PowerShell.Authentication.Core.ErrorConstants.Message.DeviceCodeTimeout, PowerShell.Authentication.Core.Constants.MaxDeviceCodeTimeOut));
                }
                      ...
                      || authContext.AuthProviderType == AuthProviderType.DeviceCodeProviderFallBack))
                {
                    ...
                    throw new MsalClientException(msalServiceEx.ErrorCode, $"{msalServiceEx.Message}.\r\n{PowerShell.Authentication.Core.ErrorConstants.Message.InvalidScope}", msalServiceEx);
                }
