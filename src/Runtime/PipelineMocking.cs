...
namespace Microsoft.Graph.PowerShell.Runtime
{
    ...
    public class PipelineMock
    {
        ...
        private Stack<string> scenario = new Stack<string>();
        ...
        public static implicit operator SendAsyncStep(PipelineMock instance) => instance.SendAsync;
        ...
        public static XImmutableArray<string> Removed = new XImmutableArray<string>(new string[] { "[Filtered]" });

        public static IEnumerable<KeyValuePair<string, JsonNode>> FilterHeaders(IEnumerable<KeyValuePair<string, IEnumerable<string>>> headers) => headers.Select(header => new KeyValuePair<string, JsonNode>(header.Key, Blacklist.Contains(header.Key) ? Removed : new XImmutableArray<string>(header.Value.ToArray())));

        public static JsonNode SerializeContent(HttpContent content) => content == null ? XNull.Instance : SerializeContent(content.ReadAsByteArrayAsync().Result);

        public static JsonNode SerializeContent(byte[] content)
        {
        ...
        public static byte[] DeserializeContent(string content)
        {
        ...
        {
            var messages = System.IO.File.Exists(recordingPath) ? Load() : new JsonObject() ?? new JsonObject();
            ...
            System.IO.File.WriteAllText(recordingPath, messages.ToString());
        }
        ...
        private JsonObject Load()
        {
            if (System.IO.File.Exists(recordingPath))
            {
                try
                {
                    return JsonObject.FromStream(System.IO.File.OpenRead(recordingPath));
                }
                catch
                {
        ...
        public HttpResponseMessage LoadMessage(string rqKey)
        {
            ...
            var response = new HttpResponseMessage
            {
                ...
                Content = new ByteArrayContent(DeserializeContent(respMessage.StringProperty("Content")))
            };
            ...
            response.RequestMessage = new HttpRequestMessage
            {
                ...
                Content = new ByteArrayContent(DeserializeContent(reqMessage.StringProperty("Content")))
            };
