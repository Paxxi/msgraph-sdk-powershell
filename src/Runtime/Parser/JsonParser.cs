...
namespace Microsoft.Graph.PowerShell.Runtime.Json
{
    public class JsonParser : IDisposable
    {
        ...
        public JsonParser(SourceReader sourceReader)
        {
            if (sourceReader == null)
                throw new ArgumentNullException(nameof(sourceReader));

            reader = new TokenReader(new JsonTokenizer(sourceReader));

            reader.Next(); // Start with the first token
        }

        public IEnumerable<JsonNode> ReadNodes()
        {
            JsonNode node;
