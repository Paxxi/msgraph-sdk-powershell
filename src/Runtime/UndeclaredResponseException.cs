...
namespace Microsoft.Graph.PowerShell.Runtime
{
    ...
    public class RestException : Exception, IDisposable
    {
        ...
        public RestException(HttpResponseMessage response)
        {
            ...
            try
            {
                ...
                var json = Json.JsonNode.Parse(ResponseBody) as Json.JsonObject;

                // see if there is an error block in the body
                json = json.Property("error") ?? json;

                { Code = If(json?.PropertyT<Json.JsonString>("code"), out var c) ? (string)c : (string)StatusCode.ToString(); }
                { message = If(json?.PropertyT<Json.JsonString>("message"), out var m) ? (string)m : (string)Message; }
                { Action = If(json?.PropertyT<Json.JsonString>("action"), out var a) ? (string)a : (string)Action; }
            }
#if DEBUG
            catch (Exception E)
            {
    ...
    public class RestException<T> : RestException
    {
        ...
        public RestException(HttpResponseMessage response, T error) : base(response)
        {
            Error = error;
        }
    }
    ...
    {
        public UndeclaredResponseException(HttpResponseMessage response) : base(response)
        {
