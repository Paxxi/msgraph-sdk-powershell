...
namespace Microsoft.Graph.PowerShell.Runtime.Json
{
    public sealed partial class JsonNumber : JsonNode
    {
        ...
        public static implicit operator ushort(JsonNumber number)
            => ushort.Parse(number.Value);

        public static implicit operator uint(JsonNumber number)
            => uint.Parse(number.Value);

        public static implicit operator ulong(JsonNumber number)
        ...
        public static implicit operator JsonNumber(float data)
            => new JsonNumber(data.ToString());
