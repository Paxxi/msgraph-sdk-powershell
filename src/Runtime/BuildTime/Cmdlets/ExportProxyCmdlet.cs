...
using static Microsoft.Graph.PowerShell.Runtime.PowerShell.MarkdownRenderer;
        ...
        protected override void ProcessRecord()
        {
            try
            {
                ...
                var variantGroups = profileGroups.SelectMany(pg => pg.Variants
                    .GroupBy(v => new { v.CmdletName, v.Ispublic })
                    .Select(vg => new VariantGroup(ModuleName, vg.Key.CmdletName, vg.Select(v => v).ToArray(),
                        Path.Combine(vg.Key.Ispublic ? publicFolder : ExportsFolder, pg.ProfileFolder), pg.ProfileName, ispublic: vg.Key.Ispublic)))
                    .ToArray();
