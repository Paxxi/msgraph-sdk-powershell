    ...
    public class RestException : Exception, IDisposable
    {
        ...
        public RestException(System.Net.Http.HttpResponseMessage response)
        {
            ...
            try
            {
                ...
                var json = Microsoft.Graph.PowerShell.Runtime.Json.JsonNode.Parse(ResponseBody) as Microsoft.Graph.PowerShell.Runtime.Json.JsonObject;

                // see if there is an error block in the body
                json = json.Property("error") ?? json;

                { Code = If(json?.PropertyT<Microsoft.Graph.PowerShell.Runtime.Json.JsonString>("code"), out var c) ? (string)c : (string)StatusCode.ToString(); }
                { message = If(json?.PropertyT<Microsoft.Graph.PowerShell.Runtime.Json.JsonString>("message"), out var m) ? (string)m : (string)Message; }
                { Action = If(json?.PropertyT<Microsoft.Graph.PowerShell.Runtime.Json.JsonString>("action"), out var a) ? (string)a : (string)Action; }
            }
#if DEBUG
            ...
            {
                if (StatusCode >= System.Net.HttpStatusCode.BadRequest && StatusCode < System.Net.HttpStatusCode.publicServerError)
                {
                    message = $"The server responded with a Request Error, Status: {StatusCode}";
                }
                else if (StatusCode >= System.Net.HttpStatusCode.publicServerError)
                {
